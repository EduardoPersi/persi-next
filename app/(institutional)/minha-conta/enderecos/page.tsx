...
export default async function Page(){const[session,token]=await Promise.all([getServerAccountSession(),getServerAccountToken()]);if(!session||!token)redirect("/entrar");const empty=(type:"billing"|"shipping",label:string)=>({id:type,type,label,firstName:"",lastName:"",company:"",address1:"",address2:"",city:"",state:"",postcode:"",country:"BR",phone:"",isPrimary:type==="shipping"});const addresses=await getCustomerWorkspaceAddresses(token).catch(()=>[empty("billing","Cobrança"),empty("shipping","Entrega")]);return <CustomerWorkspacePage title="Endereços" session={session}><p className="mb-6 text-slate-600">Gerencie os endereços de cobrança e entrega usados no WooCommerce. A estrutura permite adicionar novos tipos futuramente.</p><CustomerAddresses initialAddresses={addresses}/></CustomerWorkspacePage>}
