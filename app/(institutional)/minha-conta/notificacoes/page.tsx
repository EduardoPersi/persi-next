...
export default async function Page(){const session=await getServerAccountSession();if(!session)redirect("/entrar");return <CustomerWorkspacePage title="Notificações" session={session}><div className="rounded-xl bg-slate-50 p-8 text-center"><Bell className="mx-auto h-12 w-12 text-[#0c2d72]" aria-hidden="true"/><h2 className="mt-4 text-lg font-bold text-[#071f5c]">Nenhuma notificação no momento</h2><p className="mt-2 text-slate-600">Avisos, novidades, promoções e reposições aparecerão aqui.</p><Link href="/minha-conta/lista-espera" className="mt-5 inline-flex font-semibold text-[#0c2d72] underline">Ver lista de espera</Link></div></CustomerWorkspacePage>}
