...
interface ProductListingPageProps extends SearchPageProps {
  catalogMode?: boolean;
}
...
export async function ProductListingPage({
  ...
  const isStoreCatalog = catalogMode && !query && !selectedBrand;
  ...
  if (isBrandCatalog || isStoreCatalog) {
    const productOptions: GetProductsOptions = {
      brand:
        isBrandCatalog || isStoreCatalog ? brandIdentifier : undefined,
      category,
  ...
  const filterContextProducts = isBrandCatalog || isStoreCatalog
    ? await getFilterContextProducts({
        brand: isBrandCatalog ? brandIdentifier : undefined,
      }).catch(() => products)
    : allProducts;
  const filterData = await getSearchFilterData(filterContextProducts);
  const pathname = isStoreCatalog ? "/loja" : "/busca";
  const storeCategories = isStoreCatalog
    ? (await getAllProductCategories()).filter(
        (item) => item.parent === 0 && (item.count ?? 0) > 0,
      )
    : [];
  ...
  const clearHref = isStoreCatalog
    ? "/loja"
    : query
      ? `/busca?q=${encodeURIComponent(query)}`
      : "/busca";
  const hasCatalogContext = Boolean(query) || isBrandCatalog || isStoreCatalog;
    ...
    <>
      ...
      <main className="py-6 sm:py-8 lg:py-10">
        <Container>
          ...
          {isStoreCatalog && storeCategories.length > 0 ? (
            <div className="mt-6 rounded-lg bg-white p-4 sm:p-6">
              <HomeCategoryCarousel categories={storeCategories} />
            </div>
          ) : null}

          {!hasCatalogContext ? (
            <div className="mt-6 rounded-xl border border-slate-200 bg-white p-8 text-center">
              <h1 className="text-2xl font-bold text-primary">Busca de produtos</h1>
          ...
          {isBrandCatalog && selectedBrand?.description ? (
            ...
            >
              ...
              <p className="mt-4 whitespace-pre-line text-left leading-7 text-foreground">
