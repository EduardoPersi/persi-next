...
test("admin é server-only, autorizado por membership e paginado",async()=>{const [repo,bridge,auth]=await Promise.all([readFile(repositoryPath,"utf8"),readFile(authPath,"utf8"),readFile(new URL("../lib/admin/authorization.ts",import.meta.url),"utf8")]);assert.match(repo,/import "server-only"/);assert.match(repo,/limit \$\{pageSize\} offset/);assert.match(bridge,/requirePimAdmin/);assert.match(auth,/verifyAdminIdentity/);assert.match(auth,/identity_provider='supabase_auth'/);assert.match(auth,/admin_memberships/);assert.doesNotMatch(auth,/manage_woocommerce/);});
...
test("queries PIM preservam o contrato camelCase usado pelo painel",async()=>{const repository=await readFile(repositoryPath,"utf8");for(const alias of ['"imageUrl"','"priceMinor"','"pendingCount"','"updatedAt"','"sourceShortDescription"','"salePriceMinor"','"lastSyncedAt"'])assert.match(repository,new RegExp(alias));});
test("fila PIM usa contagens reais e filtro de dados ausentes",async()=>{const [repository,page]=await Promise.all([readFile(repositoryPath,"utf8"),readFile(new URL("../app/admin/pim/page.tsx",import.meta.url),"utf8")]);assert.match(repository,/getPimQueueCounts/);assert.match(repository,/filters\.issue === "missing"/);assert.match(page,/counts\.needsReview/);assert.match(page,/issue=missing/);});
