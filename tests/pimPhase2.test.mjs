...
test("actor vem da membership administrativa e não do formulário",async()=>{const actions=await load("app/admin/products/[id]/actions.ts");assert.match(actions,/requireAdminPermission/);assert.match(actions,/admin\.actorReference/);assert.match(actions,/auditContext\(admin\)/);assert.doesNotMatch(actions,/formData\.get\("actor/);});
