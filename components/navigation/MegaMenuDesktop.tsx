...
function NestedCategory({ category }: { category: NavigationCategory }) {
  return (
    <li className="group/nested relative">
      <Link href={category.href} className="flex min-h-9 items-center justify-between gap-2 rounded-md py-1.5 text-sm leading-5 text-[#1f2937] transition hover:text-[#ff6a00] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0c2d72]">
    ...
    <div className="columns-2 gap-x-8 xl:columns-3">
      {category.children.map((group) => (
        <section key={group.id} className="mb-6 break-inside-avoid">
          <Link href={group.href} className="block rounded-sm text-sm font-bold uppercase leading-5 text-[#071f5c] hover:text-[#ff6a00] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0c2d72]">
            {group.name}
          </Link>
  ...
  return (
    <div className="relative" onMouseEnter={cancelClose} onMouseLeave={scheduleClose}>
      <nav className="flex items-center" aria-label="Categorias de produtos">
        <button type="button" data-mega-trigger aria-expanded={openMenuId !== null} aria-controls="mega-menu-panel" onMouseEnter={() => open("all")} onFocus={() => open("all")} onKeyDown={(event) => handleTriggerKeyDown(event, "all")} className="flex min-h-12 items-center gap-2 border-r border-white/40 px-3 text-base font-semibold text-white transition hover:bg-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white lg:px-4">
        ...
        {featuredCategories.map(({ category, label }) => {
          ...
          return <Link key={category.id} href={category.href} data-mega-trigger aria-expanded={openMenuId === category.id} aria-controls="mega-menu-panel" onMouseEnter={() => open(category.id, category)} onFocus={() => open(category.id, category)} onKeyDown={(event) => handleTriggerKeyDown(event, category.id, category)} className={`flex min-h-12 items-center gap-2 border-r border-white/40 px-3 text-base font-semibold text-white transition hover:bg-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white lg:px-4 ${highlighted ? "bg-black/15 underline decoration-2 underline-offset-4" : ""}`}><CategoryIcon name={category.name} className="h-5 w-5" />{label}</Link>;
        })}
        <Link href="/promocoes" data-mega-trigger onKeyDown={(event) => handleTriggerKeyDown(event, "all")} className="flex min-h-12 items-center border-r border-white/40 px-3 text-base font-semibold text-white transition hover:bg-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white lg:px-4">Promoções</Link>
      </nav>
        ...
        <div className="grid min-h-[520px] grid-cols-[270px_minmax(0,1fr)_390px] overflow-visible rounded-b-xl">
          <aside data-category-rail className="overflow-hidden rounded-bl-xl bg-[#0c2d72] py-2 text-white">
            {menu.categories.map((category) => {
              ...
              return <Link key={category.id} href={category.href} onMouseEnter={() => setSelectedCategory(category)} onFocus={() => setSelectedCategory(category)} className={`flex min-h-14 items-center gap-3 px-5 py-3 text-base font-semibold text-white transition ${active ? "bg-white !text-[#071f5c]" : "hover:bg-[#071f5c]"}`}><CategoryIcon name={category.name} className="h-5 w-5 shrink-0" /><span className="min-w-0 flex-1 truncate">{category.name}</span><ChevronRight className="h-4 w-4 shrink-0" aria-hidden="true" /></Link>;
            })}
            <Link href="/busca" className="flex min-h-14 items-center gap-3 px-5 py-3 text-base font-semibold text-white transition hover:bg-[#071f5c]"><Grid3X3 className="h-5 w-5" aria-hidden="true" />Todos os produtos</Link>
          </aside>
          ...
          <aside className="p-7 pl-2">
            <div className="group relative h-full min-h-[450px] overflow-hidden rounded-xl bg-slate-200">
              ...
              <div className="absolute inset-x-0 bottom-0 p-7 text-white">
                ...
                <Link href={preview.href} className="mt-5 inline-flex min-h-11 items-center bg-[#0c2d72] px-5 text-sm font-bold uppercase text-white transition hover:bg-[#071f5c] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">Conheça os produtos</Link>
              </div>
            </div>
          </aside>
        </div>
