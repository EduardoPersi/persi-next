 ...
 * indeterminada e pode colapsar o rótulo a 1 caractere.
 */
      ...
      {items.map((item, index) => (
        <Fragment key={item.href}>
          ...
          <Link
            href={item.href}
            title={item.label}
            className={`tap-feedback block truncate rounded-sm px-0.5 text-sm font-medium text-muted transition-colors hover:text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
              index === items.length - 1
                ? "min-w-0 flex-1"
                : "min-w-[2.5rem] max-w-[45%] shrink"
            }`}
          >
            {item.label}
          </Link>
