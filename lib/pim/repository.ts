...
export const PIM_STATUSES = ["raw", "normalized", "needs_enrichment", "ai_suggested", "needs_review", "approved", "rejected", "published"] as const;
...
export type PimProductDetail = PimProductListItem & { slug:string; shortDescription:string|null; description:string|null; salePriceMinor:string|null; source:string; lastSyncedAt:Date|null; attributes:PimAttributeItem[]; suggestions:Array<{id:string;fieldName:string;value:string;source:string;status:string;confidence:string|null}> };
...
export async function listPimProducts(filters: PimProductFilters) {
  ...
  const rows=await db.execute(sql`select p.id,p.name,v.sku,v.gtin,b.name brand,c.name category,pm.public_url image_url,
    pr.list_amount_minor::text price_minor,pr.currency,il.quantity_available::text stock,coalesce(pp.workflow_status::text,'raw') status,
    coalesce(pending.count,0)::int pending_count,coalesce(pp.updated_at,p.updated_at) updated_at from products p ${baseJoins}
...
export async function getPimProduct(id:string):Promise<PimProductDetail|null>{
  const db=getDatabase();
  const rows=await db.execute(sql`select p.id,p.name,p.slug,p.short_description,p.description,v.sku,v.gtin,b.name brand,c.name category,pm.public_url image_url,
    pr.list_amount_minor::text price_minor,pr.sale_amount_minor::text sale_price_minor,pr.currency,il.quantity_available::text stock,
    coalesce(pp.workflow_status::text,'raw') status,coalesce(pending.count,0)::int pending_count,coalesce(pp.updated_at,p.updated_at) updated_at,
    'woocommerce' source,em.source_changed_at last_synced_at from products p ${baseJoins}
