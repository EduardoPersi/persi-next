...
export function parseBrowserNewsletterSubscription(value:unknown) {
  ...
  if(keys!=="consent,email,recaptchaToken,website"||typeof body.email!=="string"||body.email.length>254||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email.trim())||body.website!==""||body.consent!==true||typeof body.recaptchaToken!=="string"||body.recaptchaToken.length>2048) throw new Error("invalid");
